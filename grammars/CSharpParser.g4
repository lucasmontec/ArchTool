...
@header { package com.carvalab.archtool.parsers; }
...
assignment_operator
	: ASSIGNMENT | OP_ADD_ASSIGNMENT | OP_SUB_ASSIGNMENT | OP_MULT_ASSIGNMENT  | OP_DIV_ASSIGNMENT  | '%=' | '&=' | '|=' | '^=' | '<<=' | right_shift_assignment
	;
...
relational_expression
	: shift_expression ((LT | GT | '<=' | '>=') shift_expression | IS isType | AS type)*
	;
...
primary_expression_start
	...
	| BASE (DOT identifier type_argument_list? | '[' expression_list ']') #baseAccessExpression
...
bracket_expression
	: INTERR? '[' indexer_argument ( COMMA indexer_argument)* ']'
	;
...
member_initializer
	: (identifier | '[' expression ']') ASSIGNMENT initializer_value // C# 6
	;
...
isType
	: base_type (rank_specifier | STAR)* INTERR?
	;
...
using_directive
	: USING identifier ASSIGNMENT namespace_or_type_name ';'            #usingAliasDirective
...
global_attribute_section
	: '[' global_attribute_target COLON attribute_list COMMA? ']'
	;
...
attribute_section
	: '[' (attribute_target COLON)? attribute_list COMMA? ']'
	;
...
right_shift_assignment
	: first=GT second='>=' {$first.index + 1 == $second.index}? // Nothing between the tokens?
	;
...
property_declaration // Property initializer & lambda in properties C# 6
	: member_name (OPEN_BRACE accessor_declarations CLOSE_BRACE (ASSIGNMENT variable_initializer ';')? | right_arrow expression ';')
	;
...
operator_declaration // lamdas form C# 6
	: OPERATOR overloadable_operator OPEN_PARENS arg_declaration
	       (COMMA arg_declaration)? CLOSE_PARENS (body | right_arrow expression ';')
	;
